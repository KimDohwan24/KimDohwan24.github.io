...
검증된 결과로 안정성을 만듭니다.`,
    heroDescription: `탄탄한 구조 위에서 신뢰할 수 있는 서비스를 만드는 백엔드 개발자입니다.
서비스의 병목을 찾아 성능을 최적화하고, 예기치 못한 장애나 트래픽 집중 상황에서도 요청이 끊기지 않도록 안정성을 고민합니다.`,
    aboutDescription: `문제를 구조적으로 분석하고 상황에 가장 알맞은 해결책을 선택하는 과정에 집중합니다.
단순히 기능을 만드는 것에 만족하지 않고, 구현 전 완료 기준을 명확히 세우며 기술적 선택의 이유를 고민합니다.

서비스 지표와 부하 테스트를 통해 개선 전후를 객관적으로 검증합니다.
자주 발생하는 데이터 병목을 해소하고, 외부 연동 실패나 오류 상황에서도 서비스가 멈추지 않도록 예외 처리를 꼼꼼하게 설계합니다.

과정과 결과를 문서로 기록하여 동료들과 투명하게 소통합니다.
문제가 생겼을 때 빠르게 원인을 추적할 수 있는 환경을 만들고, 팀이 함께 성장할 수 있는 튼튼한 시스템 기반을 다져갑니다.`,
};
...
export const projects = [
    ...
    },
    {
        ...
        status: '완료',
        description:
            '사용자의 투자 성향과 AI 뉴스 분석을 결합한 맞춤형 주식 추천 서비스입니다. 6개의 각기 다른 성향을 가진 AI 캐릭터가 실시간 뉴스를 분석하고 모의 투자를 진행하는 독특한 시스템입니다. 외부 증권사 API나 AI 서버 연동 시 응답이 지연되거나 멈추는 현상을 방지하기 위해 비동기 처리를 도입했습니다. 또한 특정 기능에 장애가 발생해도 전체 서비스가 멈추지 않고, 마지막 정상 데이터를 보여주도록 안전장치를 구현해 서비스 안정성을 높였습니다.',
