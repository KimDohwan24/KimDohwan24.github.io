...
export const projects = [
    {
        ...
        highlights: [
            '서비스 모니터링 툴을 활용해 데이터베이스 연결(Connection) 병목 원인을 파악하고 문제 해결 경험',
    ...
    },
    {
        ...
        status: '진행중',
        description:
            '게임(로스트아크) Open API를 활용해 캐릭터 정보를 검색하는 서비스입니다. 캐릭터의 방대한 정보를 화면에 보여줄 때 발생하는 성능 저하 문제를 해결하기 위해 필요한 정보만 가공하고, 화면에 보이는 부분만 우선적으로 로딩되도록 설계했습니다. 또한, 외부 API의 호출 횟수 제한 문제를 극복하기 위해 자주 변하지 않는 데이터는 로컬 캐시(Caffeine)에, 검색 결과는 글로벌 캐시(Redis)에 저장하는 이중 캐싱 전략으로 검색 속도를 크게 높였습니다.',
        tags: ['React', 'Vite', 'JavaScript', 'CSS', 'Spring Boot'],
        iconName: 'Globe',
        iconColor: '#38bdf8',
        link: 'https://github.com/KimDohwan24/LostArkSearch',
        highlights: [
            '외부 API의 한정된 호출 횟수를 효율적으로 사용하기 위해 Redis와 로컬 캐시를 결합한 이중 캐싱 환경 구축',
            '크고 복잡한 데이터를 화면에 그리기 쉽게(렌더링 최적화) 필요한 정보만 담은 객체(DTO)로 변환하여 성능 개선',
            '모바일 환경에서도 끊김 없이 부드럽게 작동하도록 반응형 웹 디자인(UI/UX) 구현'
        ]
    },
    {
        ...
        highlights: [
            ...
            '분리된 환경에서 에러 발생 시 데이터 불일치를 막기 위해 실패한 작업을 되돌리는 복구 로직(보상 트랜잭션) 구현',
            '대량의 데이터를 저장할 때 JPA 대신 쿼리를 직접 다루는 방식(JdbcTemplate 묶음 저장)을 선택해 성능 93% 향상',
            '사용자 모두에게 알림을 보낼 때 발생하는 지연 문제를 해결하기 위해 메시지 전송 구조 최적화',
            '선착순 할인 이벤트 시 여러 명이 동시에 구매를 요청할 때 발생하는 재고 수량 오류를 방지하기 위해 분산 락(Redisson) 적용'
        ]
    },
    {
        title: 'paw-go',
    ...
    },
    {
        ...
        highlights: [
            ...
            '외부 증권사 API를 여러 번 호출할 때 생기는 대기 시간을 줄이기 위해 병렬 처리(비동기)를 적용하여 빠른 응답성 확보',
            '외부 통신 객체(WebClient)를 재사용할 수 있도록 최적화하여 서버 자원 낭비와 연결 오류(커넥션 풀 고갈) 방지',
            '프론트엔드와 백엔드 간의 불필요한 통신 횟수를 줄이기 위해 서버에서 데이터를 모아서 한 번에 전달하도록 개선 (초기 로딩 속도 50% 단축)',
            '외부 API에 문제가 생겨도 사용자는 에러 화면 대신 마지막 정상 주식 가격을 볼 수 있도록 안전망(Circuit Breaker) 적용'
        ]
    },
];
