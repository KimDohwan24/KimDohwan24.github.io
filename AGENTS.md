...
### 2.1 브랜치 및 커밋/푸시 규칙 (Branching, Commit & Push Rules)
- **1인 개발 간소화 전략 (Solo Dev Flow):**
  - 모든 기능 개발, 버그 수정, 스타일 개선 작업은 **`main` 브랜치에서 직접 작업**합니다.
  - **커밋(Commit) 제한:** 사용자가 명시적으로 `commit`을 진행하라고 지시하기 전까지는 **절대로 임의로 `git commit`을 실행하지 마십시오.** (파일 수정 상태로 유지)
  - **푸시(Push) 제한:** 사용자가 명시적으로 `push`를 진행하라고 지시하기 전까지는 **절대로 임의로 원격 저장소에 `git push`하지 마십시오.**
  - 사용자의 명시적인 지시가 있을 때만 해당 단계의 Git 명령어를 실행합니다.
